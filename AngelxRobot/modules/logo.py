...
from AngelxRobot import OWNER_ID
from AngelxRobot.events import register
from AngelxRobot import telethn
...
async def lego(event):
    ...
    try:
        ...
        await telethn.send_file(
            event.chat_id, file=fname, caption=f"ʟᴏɢᴏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʙʏ 𝗔𝗡𝗚𝗟𝗘"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception as e:
        await event.reply(f"ғʟᴏᴏᴅᴡᴀɪᴛ ᴇʀʀᴏʀ, ʀᴇᴩᴏʀᴛ ᴛʜɪs ᴀᴛ @love_u_bobby")


__mod_name__ = "Lᴏɢᴏ💫"

__help__ = """
Angel Robot can create some beautiful and attractive logo for your profile pics.
